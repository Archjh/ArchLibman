...
//		WorldRenderer worldrenderer = tessellator.getWorldRenderer();
	...
	public static void drawRoundedRect(int x, int y, int width, int height, int radius, int color) {
		// 绘制中心矩形
		Gui.drawRect(x + radius, y + radius, width - radius, height - radius, color);

		// 绘制四个角
		drawFilledCircle(x + radius, y + radius, radius, color); // 左上
		drawFilledCircle(width - radius, y + radius, radius, color); // 右上
		drawFilledCircle(x + radius, height - radius, radius, color); // 左下
		drawFilledCircle(width - radius, height - radius, radius, color); // 右下

		// 绘制四条边
		Gui.drawRect(x + radius, y, width - radius, y + radius, color); // 上边
		Gui.drawRect(x, y + radius, x + radius, height - radius, color); // 左边
		Gui.drawRect(width - radius, y + radius, width, height - radius, color); // 右边
		Gui.drawRect(x + radius, height - radius, width - radius, height, color); // 下边
	}
	 ...
	 * @param color 颜色 (RGB)
	 */
	// 在 RenderUtils.java 中添加或修改以下方法
	public static void drawStringWithShadow(String text, int x, int y, int color) {
		// 使用支持中文的字体渲染器
		UnicodeFontRenderer fontRenderer = BaseClient.instance.getFontRenderer().getFont();
