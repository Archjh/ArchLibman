...
public class TabGui1 extends EventListener {
	...
	private void renderMenu(List<String> items, int currentItem) {
		int height = mc.fontRendererObj.FONT_HEIGHT + 2; // 使用原生字体高度

		// 绘制标题
		mc.fontRendererObj.drawStringWithShadow(
				String.format("%1$s - %2$s", BaseClient.instance.getClientName(), BaseClient.instance.getClientVersion()),
				5, 2,
				Color.WHITE.getRGB()
		);
		...
		for (String item : items) {
			int itemWidth = mc.fontRendererObj.getStringWidth(item);
			if (itemWidth > maxItemWidth)
				maxItemWidth = itemWidth;
		}

		// 绘制背景框
		int boxWidth = maxItemWidth + 10;
		int boxHeight = height * items.size();
		int boxX = 5;
		int boxY = height + 2;

		// 绘制紫色左侧线
		RenderUtils.drawRect(boxX, boxY, boxX + 2, boxY + boxHeight, new Color(128, 0, 128).getRGB());
		// 绘制黑色背景
		RenderUtils.drawRect(boxX + 2, boxY, boxX + boxWidth, boxY + boxHeight, new Color(0, 0, 0, 255).getRGB());
		...
		for (int i = 0; i < items.size(); i++) {
			...
			// 绘制选中项的背景
			if (isCurrentItem) {
				RenderUtils.drawRect(boxX + 2, boxY + i * height, boxX + boxWidth, boxY + (i + 1) * height, new Color(50, 50, 50).getRGB());
			}
			...
			mc.fontRendererObj.drawStringWithShadow(
					...
					boxX + 5,
					boxY + i * height + 1,
					isCurrentItem ? new Color(200, 0, 200).getRGB() : Color.WHITE.getRGB()
			);
		}
		...
		if (indentation == 1 && getCurrentModule() != null) {
			...
			int descX = boxX + boxWidth + 5;
			int descY = boxY;

			// 绘制描述背景
			RenderUtils.drawRect(descX, descY, descX + descWidth + 4, descY + height, new Color(0, 0, 0, 255).getRGB());
			// 绘制描述文本
			mc.fontRendererObj.drawStringWithShadow(description, descX + 2, descY + 1, Color.WHITE.getRGB());
		}
	}
}